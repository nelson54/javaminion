  ...
  stages {
    ...
    stage('Waiting for server') {
      steps {
        waitUntil {
          script {
            def statusCode = sh script: 'timeout 1 bash -c \'while [[ "$(curl -s -o /dev/null -w \'\'%{http_code}\'\' https://dominion.dereknelson.io)" != "200" ]]; do sleep 1; done;\' || false', returnStatus: true
            return (statusCode == 0)
          }
        }
      }
